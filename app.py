...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "C:\Projects\Python\pubsub1\cbd3354-435500-54b00e8204c8.json"
...
def start_listening():
    ...
    try:
        streaming_pull_future.result(timeout=300)  # Listen for messages for 5 minutes
    except TimeoutError:
...
# Route to display messages from the database
@app.route('/')
def index():
    connection = connect_to_db()
    cursor = connection.cursor()
    ...
    cursor.execute("SELECT * FROM pubsub_messages ORDER BY received_at DESC")
